...
def main():
    global FPSCLOCK, DISPLAYSURF, BASICFONT, RESET_SURF, RESET_RECT, NEW_SURF,\
        NEW_RECT, SOLVE_SURF, SOLVE_RECT, COUNT_SURF, COUNT_RECT, totalStep
    ...
    COUNT_SURF, COUNT_RECT = makeText('Steps: ' + str(0), TEXTCOLOR, BGCOLOR,
                                      WINDOWWIDTH - 120, 5)
...
def getStartingBoard():
    ...
    for x in range(BOARDWIDTH):
        ...
        for y in range(BOARDHEIGHT):
            ...
            counter += BOARDWIDTH
        board.append(column)
        counter -= BOARDWIDTH * (BOARDHEIGHT - 1) + BOARDHEIGHT - 1
...
def drawBoard(board, message):
    ...
    COUNT_SURF, COUNT_RECT = makeText('Step: ' + str(totalStep), MESSAGECOLOR, BGCOLOR, WINDOWWIDTH - 100, 5)
    for tileX in range(len(board)):
        for tileY in range(len(board[0])):
