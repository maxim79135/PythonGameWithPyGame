...
ALLSHAPES = (DONUT, SQUARE, DIAMONG, LINES, OVAL)
assert len(ALLCOLORS) * len(ALLSHAPES) * 2 >= BOARDWIDTH * BOARDHEIGHT,\
    "Board is too big for the number of shapes/colors defined."

def main():
    global FPSCLOCK, DISPLAYSURF, count, text, isStarted
    pygame.init()
            ...
            if not revealedBoxes[boxx][boxy] and mouseClicked:
                ...
                if firstSelection == None:
                    ...
                    if icon1shape != icon2shape or icon1color != icon2color:
                        ...
                        startGameAnimation(mainBoard)
                    firstSelection = None
        DISPLAYSURF.blit(font.render(text, True, (0, 0, 0)), (0, 0))
        pygame.display.update()
        FPSCLOCK.tick(FPS)

def generateReveleadBoxesData(val):
    revealedBoxes = []
...
def getRandomizeBoard():
    ...
    numIconUsed = int(BOARDWIDTH * BOARDWIDTH / 2)
...
def startGameAnimation(board):
    coveredBoxes = generateReveleadBoxesData(False)
    boxes = []
    for x in range(BOARDWIDTH):
        for y in range(BOARDHEIGHT):
            boxes.append((x, y))
...
def gameWonAnimation(board):
    coveredBoxes = generateReveleadBoxesData(True)
    color1 = LIGHTBGCOLOR
    color2 = BGCOLOR
